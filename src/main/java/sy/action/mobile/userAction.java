...
public class userAction extends BaseAction<Syuser> {
	...
	synchronized public void doNotNeedSessionAndSecurity_reg() {
		...
		if (user != null) {
			...
			u.setSalt(salt);
			u.setPwd(MD5Util.md5(data.getPwd() + salt));
			service.save(u);
	...
	public void doNotNeedSessionAndSecurity_login() {
		...
		signParams.add(new NameValuePair("data.loginname", data.getLoginname()));
		signParams.add(new NameValuePair("data.pwd", data.getPwd()));
		
		String sign = VerifySignUtil.sign(signParams);
		if (StringUtils.isEmpty(name) || StringUtils.isEmpty(pwd)) {
			json.setMsg("缺少参数！");
		} else if (!sign.equals(data.getSign())) {
			json.setMsg("签名验证失败！");
		} else {
			HqlFilter hqlFilter = new HqlFilter();
			hqlFilter.addFilter("QUERY_t#loginname_S_EQ", data.getLoginname());
			...
			if (user != null) {
				...
				hqlFilter.addFilter("QUERY_t#pwd_S_EQ",
						MD5Util.md5(data.getPwd() + salt));
				user = service.getByFilter(hqlFilter);
				if (user != null) {
					...
					for (Syorganization organization : user
							.getSyorganizations()) {
	...
	public void doNotNeedSecurity_updateCurrentPwd() {
		...
		String salt = UUID.randomUUID().toString().replace("-", "");
		user.setPwd(MD5Util.md5(data.getPwd() + salt));
	...
	synchronized public void save() {
		...
		if (data != null) {
			...
			if (user != null) {
				...
				Syuser newUser = data;
				String salt = UUID.randomUUID().toString().replace("-", "");
				newUser.setPwd(MD5Util.md5("123456") + salt);
